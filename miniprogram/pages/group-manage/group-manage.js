...
Page({
  data: { projectId: '', groups: [], loading: true, operating: false },
  onLoad(query) { this.setData({ projectId: query.projectId || '' }); },
  onShow() { if (this.data.projectId) this.load(); },
  async load() { const res = await service.list(this.data.projectId); this.setData({ groups: res.success ? res.data.groups : [], loading: false }); if (!res.success) wx.showToast({ title: res.message, icon: 'none' }); },
