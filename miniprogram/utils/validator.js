...
function validateProjectTitle(title) {
  ...
  if (t.length > LIMITS.PROJECT_TITLE_MAX) return 事件名称不能超过字;
  return '';
}
...
function validateTaskTitle(title) {
  ...
  if (t.length > LIMITS.TASK_TITLE_MAX) return 任务名称不能超过字;
  return '';
}

function validateNote(note) {
  if (!note) return '';
  if (note.length > LIMITS.TASK_NOTE_MAX) return 备注不能超过字;
  return '';
}

function validateProjectDesc(desc) {
  if (!desc) return '';
  if (desc.length > LIMITS.PROJECT_DESC_MAX) return 说明不能超过字;
  return '';
}
...
function validateGroupName(name) {
  ...
  if (n.length > LIMITS.GROUP_NAME_MAX) return 分组名称不能超过字;
  return '';
}
