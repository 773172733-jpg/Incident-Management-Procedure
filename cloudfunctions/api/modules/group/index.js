...
function groupInput(payload) {
  ...
  return { data: { name: name.value, color: typeof payload.color === 'string' ? payload.color.slice(0, 16) : '#FF6B35', icon: typeof payload.icon === 'string' ? payload.icon.slice(0, 24) : 'folder' } };
}
async function list(payload, context) {
  ...
  const project = await projectFor(openid, id.value); if (!project) return fail('NOT_FOUND', '事件不存在或无权访问');
...
async function create(payload, context) {
  ...
  const project = await projectFor(openid, id.value); if (!project) return fail('NOT_FOUND', '事件不存在或无权操作');
  ...
  await writeActivityLog({ projectId: project._id, operatorId: openid, action: 'group.created', targetType: 'group', targetId: result._id, targetTitleSnapshot: doc.name, after: doc, visibleTo: [openid] });
...
async function update(payload, context) {
  ...
  const project = group && await projectFor(openid, group.projectId); if (!group || !project || group.deletedAt) return fail('NOT_FOUND', '分组不存在或无权操作');
  await db.collection('project_groups').doc(group._id).update({ data: { ...parsed.data, updatedAt: db.serverDate() } });
  await writeActivityLog({ projectId: project._id, operatorId: openid, action: 'group.updated', targetType: 'group', targetId: group._id, targetTitleSnapshot: parsed.data.name, before: { name: group.name }, after: parsed.data, visibleTo: [openid] });
...
async function remove(payload, context) {
  ...
  await writeActivityLog({ projectId: project._id, operatorId: openid, action: 'group.deleted', targetType: 'group', targetId: group._id, targetTitleSnapshot: group.name, visibleTo: [openid] });
...
async function reorder(payload, context) {
  ...
  await writeActivityLog({ projectId: project._id, operatorId: openid, action: 'group.reordered', targetType: 'group', targetTitleSnapshot: '分组排序', after: { groupIds }, visibleTo: [openid] });
