...
function trimAndCheck(str, maxLen) {
  ...
  if (trimmed.length > maxLen) return { valid: false, message: 不能超过字 };
  return { valid: true, value: trimmed };
}

function validateProjectTitle(title) {
  const r = trimAndCheck(title, LIMITS.PROJECT_TITLE_MAX);
  if (!r.valid) return { valid: false, message: 事件名称 };
...
function validateTaskTitle(title) {
  ...
  if (!r.valid) return { valid: false, message: 任务名称 };
...
function validateGroupName(name) {
  ...
  if (!r.valid) return { valid: false, message: 分组名称 };
